...
const declare = async () => {
    try {
        ...
        const compiledContract = json.parse(fs.readFileSync("../target/dev/factory_anchor_Anchoring.sierra.json").toString("ascii"));
        const compiledContractCasm = json.parse(fs.readFileSync("../target/dev/factory_anchor_Anchoring.casm.json").toString("ascii"));
